...
public class LastDayWhereYouCanStillCross {
    ...
    public static int latestDayToCross(int row, int col, int[][] cells) {
        ...
        int days = 0;
        // using binary search.
        // 1) when we know that we can cross at some day, the last crossing day would be
        // in the right half.
        // 2) when we know that we can not cross at some day, the last crossing day
        // would be in the left half.
    ...
    public static boolean topToBottomWalkPossible(int[][] cells, int row, int col, int dayAt) {
        ...
        for (int c = 0; c < col; c++) {
            // row should be always zero and the cell value should be zero (that's when we
            // can start from that cell to cross).
            // we will add those cells into the queue as starting point for the BFS.
            // once we add that cell into the queue, we will change it's value to 1 (to
            // avoid re - processing)
            if (grid[0][c] == 0) {
