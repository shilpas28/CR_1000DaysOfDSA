    ...
    public static ArrayList<Integer> findOrder(int n, int[][] prerequisites) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int k : edges.get(e)) // go to all the edges connected to node
                if (--c[k] == 0)
                    q.add(k); // if in degress for the element is zero add that in the queue
        }
        
        if (ans.size() != n)
            return new ArrayList<Integer>(); // check if the size of answer is total no of edges means no loop exist
        return ans;
    }
}
